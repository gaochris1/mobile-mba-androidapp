...
The active metric type 'JHTTPPOSTMT' describes measurement results of the
active test for upload performance.
...
The field contains the total bytes uploaded across all connections during the
...
The active metric type 'JUDPLATENCY' describes measurement results of the
active test for latency performance.

datetime

String

Fri Jan 25 15:36:07 GMT 2013

The field contains the time test finished in UTC.
...
The field contains the number of lost packets during the test
...
The field contains the average RTT in microseconds
...
The field contains the maximum RTT in microseconds
...
The field contains the minimum RTT in microseconds
...
The field contains the standard deviation RTT measured in microseconds
...
The field contains the number of successes (note: use
...
The field holds a string of the measurement server target hostname or IP
address. The value is pulled from the test configuration file.
...
The active metric type 'CLOSESTTARGET' contains the nearest (in latency)
measurement server that the client assess as it activates for testing
...
The field contains the IP address of the nearest measurement server
...
The passive metric type 'phone_identity' describes features of the handset and
...
api android.os.Build.MANUFACTURER

The field holds a string from the Android method that identifies the handset
manufacturer.
...
The field holds a string for the Operating System of the handset. This value
is set by the application logic.
...
The field contains the unix timestamp of the handset performing the
measurement at the beginning of the observations as a sql TIMESTAMP datatype.
http://developer.android.com/reference/java/sql/Timestamp.html
...
The passive metric type 'network_data' describes features of the wireless
connectivity of the active network connection.
...
The field holds an integer from the Android method that identifies the type of
wireless network that provides Internet connectivity at the time of the
observation.

active_network_type_code
...
The field holds string from the Android method that identifies the type of
wireless network that provides Internet connectivity at the time of the
observation.
...
The field holds boolean true or false value from the Android method that
identifies whether network connectivity exists and it is possible to establish
...
The field holds string from the Android method that identifies the numeric
name (MCC+MNC) of the current registered operator of the Internet connectivity
...
The field holds string from the Android method that identifies the text
readable name of the current registered operator of the Internet connectivity
at the time of the observation.
...
The field holds an integer from the Android method that identifies a constant
indicating the radio technology (network type) currently in use on the handset
...
api android.telephony.TelephonyManager  
.getPhoneType()

The field holds an integer from the Android method that identifies a constant
indicating the device phone type.
...
The field holds a string converted from the Android method that identifies a
constant indicating the radio technology (network type) currently in use on
the handset for data transmission.
...
The field holds a string converted from the Android method that identifies a
constant indicating the device phone type.
...
The field holds a boolean true false value from the Android method that
identifies whether the active connection is considered roaming on the current
...
The field holds string from the Android method that identifies the Service
Provider Name (SPN)
...
The passive metric type 'gsm_cell_location' describes the location of handset
determined by the GSM provider technology.
...
The field holds an integer from the Android method that identifies the GSM bit
error rate (0-7, 99) as defined in TS 27.007 8.5.

cell_tower_id

Integer

api android.telephony.gsm.GsmCellLocation  
.getCid()

The field holds an integer from the Android method that identifies the GSM
tower location id, noting -1 if unknown, and 0xffff as the max allowable
value.
...
The field holds an integer from the Android method that identifies the GSM
location area code, noting -1 if unknown, and 0xffff as the max allowable
value..
...
The field holds an integer from the Android method that identifies the GSM
Signal Strength, with valid values are (0-31, 99) as defined in TS 27.007 8.
...
The field holds an integer from the Android method that identifies the primary
scrambling code of the serving cell on a UMTS network.
...
The passive metric type 'cdma_cell_location' describes the location of handset
determined by the CDMA provider technology.
...
The field holds an integer from the Android method that identifies the cdma
base station identification number, and -1 if unknown.
...
The field holds an integer from the Android method that identifies the cdma
latitude as a decimal number as specified in 3GPP2 C.S0005-A v6.0.
(http://www.3gpp2.org/public_html/specs/C.S0005-A_v6.0.pdf) It is represented
in units of 0.25 seconds and ranges from -1296000 to 1296000, both values
inclusive (corresponding to a range of -90 to +90 degrees). Integer.MAX_VALUE
is considered invalid value.
...
(http://www.3gpp2.org/public_html/specs/C.S0005-A_v6.0.pdf) It is represented
in units of 0.25 seconds and ranges from -2592000 to 2592000, both values
inclusive (corresponding to a range of -180 to +180 degrees).
...
The field holds an integer from the Android method that identifies the cdma
system identification number, and -1 if unknown.td>
...
The field holds an integer from the Android method that identifies the cdma
network identification number, and -1 if unknown.
...
api android.telephony.SignalStrength  
.getCdmaEcio()

The field holds an integer from the Android method that identifies the CDMA
Ec/Io value in dB*10
...
The passive metric type 'cell_neighbour_tower_data' describes the location of
handset determined by the GSM provider technology. Calls to this constructor
require a radio network type to specified, and responses to the respective
Android API calls return the values from the appropriate access network
methods.
...
api android.telephony.NeighboringCellInfo  
.getCid()

The field holds an integer from the Android method that identifies the
neighboring cell tower location id, noting -1 if unknown, and 0xffff as the
max allowable value.
...
The field holds an integer from the Android method that identifies the
neighboring cell tower location area code.
...
The field holds an integer from the Android method that identifies the
neighboring cell tower network type while neighboring cell location is stored.
Return TelephonyManager.NETWORK_TYPE_UNKNOWN means that the location
information is unavailable. Return TelephonyManager.NETWORK_TYPE_GPRS or
TelephonyManager.NETWORK_TYPE_EDGE means that Neighboring Cell information is
stored for GSM network, in which NeighboringCellInfo  
.getLac and NeighboringCellInfo  
.getCid should be called to access location. Return
...
means that Neighboring Cell information is stored for UMTS network, in which
NeighboringCellInfo  
...
The field holds a string converted from the Android method that identifies the
neighboring cell tower network type while neighboring cell location is stored.
...
Received signal strength or UNKNOWN_RSSI if unknown For GSM, it is in "asu"
ranging from 0 to 31 (dBm = -113 + 2*asu) 0 means "-113 dBm or less" and 31
means "-51 dBm or greater" For UMTS, it is the Level index of CPICH RSCP
defined in TS 25.125
...
The field holds an integer from the Android method that identifies the
neighboring cell tower Primary Scrambling Code in 9 bits format in UMTS, 0x1ff
max value UNKNOWN_CID if in GSM or CMDA or unknown.
...
The passive metric type 'location' describes the location of the handset
determined by network provider technology or GPS at the time of the
observation
...
The field holds a float from the Android method that identifies the accuracy
of this location, in meters.

datetime

String

Thu Jan 24 22:40:05 EST 2013
...
The field holds a double from the Android method that identifies the latitude,
in degrees.
...
The field holds a string that identifies whether location was derived from the
cellular network technology or the handset's GPS.
...
The field holds a double from the Android method that identifies the
...
The passive metric type 'PARAM_EXPIRED' is a metric which tells that value of
a parameter has been expired. At the time being it is used to get control the
value of the closest target.

datetime

String

Fri Jan 25 10:22:13 EST 2013
...
The field holds boolean true or false value that identifies whether the test
execution expired.
...
The passive metric type 'NETACTIVITY' describes the traffic sent and received
by the handset during a test condition period.
...
The field holds an integer value that identifies the number of bytes received
by the handset during the test condition period.
...
The field holds an integer value that identifies the number of bytes sent by
the handset during the test condition period.

datetime

String

Fri Jan 25 10:23:21 EST 2013
...
The field holds an integer value that identifies the maximum limit of bytes to
be received by the handset during the test condition period.
...
The field holds an integer value that identifies the maximum limit of bytes to
be sent by the handset during the test condition period.
...
The field holds a boolean value that identifies whether the condition is met.
Meaning that bytes received during the test condition period are less than
maxbytesin and the bytes sent during the test condition period are less than
maxbytesout.
...
The passive metric type 'CPUACTIVITY' describes the cpu activity of the
...
The field holds an integer value that identifies the maximum limit of cpu
activity on the handset during the test condition period.
...
The field holds an integer value that identifies the read average of the
handset's cpu activity handset during the measurement observation calculated
from /proc/stat in CpuUsageReader.java.
...
The field holds a boolean value that identifies whether the cpu activity has
been lower of max_average during the test condition period.
